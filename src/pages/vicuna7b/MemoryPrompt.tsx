...
function send(prompt: string, 
  ...
  username: string
   ...
   postData("http://demo.ella.school:5011/json/write/vicuna/7b/memory", 
   {prompt, temperature, top_p, max_new_tokens, chat_prompt_size, repetition_penalty, top_k, ainame, username});
}

function Vicuna7bMemoryPrompt(props: any) {
  const text = "child name must be {CHILDNAME}, ai name must be {AINAME}, the chat memory must be {MEMORY} (required), your input must be {INPUT} (required), last N turns must be {CHAT_HISTORY}"
  ...
  if(!received) getData("http://demo.ella.school:5011/json/read/vicuna/7b/memory").then(rsp=> rsp.json()).then(data=>{
  ...
  return (
    <div style={{
      ...
      >
        <h2 style={{ textAlign: "center", fontFamily: "Optima, sans-serif"}}>Memory Prompt</h2>
...
export default Vicuna7bMemoryPrompt;