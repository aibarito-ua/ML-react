...
function send(prompt: string, 
  ...
  ainame: string,
  username: string){
   postData("http://demo.ella.school:5011/json/write/vicuna/7b/openai", 
   {prompt, temperature, top_p, max_new_tokens, chat_prompt_size, repetition_penalty, top_k, ainame, username});
}

function OpenAIPDFPrompt(props: any) {
  const text = "child name must be {CHILDNAME}, ai name must be {AINAME}, document context must be {DOCUMENTS} (required), your input must be {INPUT} (required), last N turns is {CHAT_HISTORY}"
  ...
  if(!received) getData("http://demo.ella.school:5011/json/read/vicuna/7b/openai").then(rsp=> rsp.json()).then(data=>{
  ...
  return (
    <div style={{
      ...
      >
        <h2 style={{ textAlign: "center", fontFamily: "Optima, sans-serif"}}>OpenAI Pdf Prompt</h2>
...
export default OpenAIPDFPrompt;