...
const URL = process.env.REACT_APP_VICUNA_7B_PDF;
...
async function MainProcess(
  ...
  return await getData("http://demo.ella.school:5011/json/read/vicuna/7b/pdf").then(rsp=> rsp.json()).then(async data=>{
...
const readJson = async () => {
  return await getData("http://demo.ella.school:5011/json/read/vicuna/7b/chat/pdf").then(rsp=> rsp.json()).then(data=>{
...
const saveJson = (json) =>{
  ...
  postData("http://demo.ella.school:5011/json/write/vicuna/7b/chat/pdf", 
  {history: history});
}


class ChatPdf extends React.Component {
  constructor() {
    super();
    this.state = {
      messages: [],
